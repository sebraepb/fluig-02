function createDataset(fields, constraints, sortFields) {
    ...
    try{
		...
		var result = authService.realizarConsultaSQL("Fluig.SQL.09", 1 , "T", "");	
		...
		dataset.addColumn("CLIENTAFANTASIA");
		dataset.addColumn("CGCCFO");

		for each(consql in xmlResultados.Resultado) {
			dataset.addRow(new Array(
					consql.CODCFO.toString(),
					consql.CLIENTE.toString(),
					consql.CLIENTAFANTASIA.toString(),
					consql.CGCCFO.toString()
			));        
		}
