function createDataset(fields, constraints, sortFields) {
    ...
    try{
		...
		var result = authService.realizarConsultaSQL("Fluig.SQL.10", 1 , "T", "CODPROJETO="+filtro);	
		...
		dataset.addColumn("CODPROJETOACAO");
		dataset.addColumn("CODCCUSTO");
		dataset.addColumn("CODACAO");
		dataset.addColumn("NOMEACAO");
		dataset.addColumn("SALDOACAO");

		for each(consql in xmlResultados.Resultado) {
			dataset.addRow(new Array(
					consql.CODPROJETO.toString(),
					consql.CODCCUSTO.toString(),
					consql.CODACAO.toString(),
					consql.NOMEACAO.toString(),
					consql.SALDOACAO.toString()
			));        
		}
