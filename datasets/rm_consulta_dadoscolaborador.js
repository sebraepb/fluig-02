function createDataset(fields, constraints, sortFields) {
    ...
    try{
		...
		var result = authService.realizarConsultaSQL("Fluig.SQL.03", 1 , "T", "CODPESSOA="+filtro);	
		...
		dataset.addColumn("SECAO");
		dataset.addColumn("GERENTE");
		dataset.addColumn("GERENTEUSUARIO");
		dataset.addColumn("DIRETOR");
		dataset.addColumn("CODVEN");
		
		for each(consql in xmlResultados.Resultado) {
			dataset.addRow(new Array(
					consql.NOME.toString(),
					consql.CODFILIAL.toString(),
					consql.CHAPA.toString(),				
					consql.SECAO.toString(),
					consql.GERENTE.toString(),
					consql.GERENTEUSUARIO.toString(),
					consql.DIRETOR.toString(),
					consql.CODVEN.toString()
			));        
		}
